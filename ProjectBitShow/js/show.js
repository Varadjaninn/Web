...
const showCast = (id) => {
  $.ajax({
    ...
    response.forEach((element) => {
      let castMembers = `<li>${element.person.name}</li>`;
      $(".cast").append(castMembers);
    });
  });
};

//Adding crew to the list
...
const showCrew = (id) => {
  $.ajax({
    ...
    response.forEach((element) => {
      let crewMembers = `<li>${element.person.name}</li>`;
